...
def nonDimODE(u, nn_dt, ts):
    ...
    odes = torch.zeros_like(u, device=device)
    D_1_t = nn_dt[:,0]
    D_2_t = nn_dt[:,1]
    I_sc_t = nn_dt[:,2]
    I_p_t = nn_dt[:,3]
    ...
    odes[:,0] = D_1_t - (ds * t_s) / D_1s + t_s / taum * D1
    odes[:,1] = D_2_t - (t_s * D_1s) / (taum * D_2s) * D1

    odes[:,2] = I_sc_t - (t_s * us) / (tau1 * Ci * I_scs) + (t_s / tau1) * I_sc
    odes[:,3] = I_p_t - (t_s * I_scs) / (tau2 * I_ps) * I_scs + (t_s / tau2) * I_p
    pdes[:,4] = I_eff_t + p2 * t_s * I_eff - ((p2 * Si * I_ps * t_s) / I_effs) * I_p
    
    odes[:,5] = G_t + (t_s * GEZI + t_s * I_effs * I_eff) * G + (t_s * EGP0) / G_s + (1000 * t_s * D_2s) / (Vg * taum * G_s) * D2
    odes[:,6] = G_sc_t - (G_s * t_s) / (G_scs * tausc) * G + (t_s / tausc) * G_sc
...
class PINN():
    ...
    def data_loss(self, ts, data):
        '''Only trains on data from G, remember  to scale predictions from NN accordingly'''
        G_s = 454.54
        u = self.nn(ts)
        G_pred = u[:,5] * G_s
        loss = G_pred - data[:,5]
